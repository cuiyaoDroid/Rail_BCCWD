...
public abstract class BaseRongYunService extends Service {
	...
	public void onCreate() {
		...
		buleng = sp.load(getApplicationContext(), R.raw.buleng, 1);
	}
	...
	protected void notifyClient(int icon,String fromJid, String fromUserName,
			String message,Intent mNotificationIntent) {
		mWakeLock.acquire();
		setNotification(icon,fromJid, fromUserName, message,mNotificationIntent);
		setLEDNotification();
	...
	private void setNotification(int icon,String fromJid, String fromUserId,
			String message,Intent mNotificationIntent) {
		...
		PendingIntent pendingIntent = PendingIntent.getActivity(this, Integer.parseInt(fromJid),
